...
public class ShorterServiceImpl implements ShorterServiceApi {
  ...
  public ShorterServiceImpl(UrlRepository repository, UrlEncoderApi urlEncoder, ModelMapper mapper,
  ...
  public UrlDTO makeShort(UrlDTO urlDTO) {
    ...
    try {
      ...
      throw new InternalException(ex.getMostSpecificCause().getMessage());
    }
  ...
  public Optional<UrlDTO> getShort(String code) {
    return repository.findOneByCode(code).map(c -> mapper.map(c, UrlDTO.class));
  }
